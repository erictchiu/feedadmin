...
if ($ccount) { 
	...
	// noscript tag
	print "
	<div class='tb-small tb-center'><noscript>{$lang['noscript']}</noscript></div>";
	
//	++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// Print the search form
//	++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

	if ($config['comment_search']) print "
	<!-- Search -->
	<form id='tb-search-form' action='".COMMENTS_DRIVER."'  method='post'>
		$common
		<input class='tb-search-field' type='text' name='search_terms' value='' />
		<input class='tb-submit' type='submit' name ='search' value ='{$lang['search']}' onmouseout=\"this.className='tb-submit'\" onmouseover=\"this.className='tb-submit-hover'\" />
		$search_help
	</form>";
...
//	++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	if ($config['comments_legend']) {
		if ($config['use_pages']) {
				print "
	<!-- n of n pages -->
	<div class='tb-comment-legend'>{$lang['comdisplay1a']}</div>";
		} else {
			if ($total_rows > 0 && $config['comments_legend']) {
				if ($total_rows > $num_rows) {
				
					print "
	<!-- Showing n-n of n comments -->
...
//	+++++++++++++++++++++++++++++++++++++++++++++++
	for($i=0; $i<$ccount; $i++) {
		...
		if ($config['gravatar']) {
			$gravatar_image = "<img class='tb-gravatar' src='{$grav_url[$i]}' height='{$config['gravatar_size']}' width='{$config['gravatar_size']}' alt='gravatar' />";
		} else {
			$gravatar_image = '';
		}
	...
	<div id='ID-$tbid[$i]' $comment_type_class[$i]>
		...
		<div class='$header_class[$i]'> 
			<div class='$author_loc_class[$i]'>
				$gravatar_image <span class='$author_class[$i]'>$author[$i]</span> 
